...
function log() {
    ...
    $.fn.uploadFile = function (options) {
        ...
        var uploadLabel = $('<div class="btn btn-primary">' + s.uploadStr + '</div>');
	   ...
	   this.startUpload = function () {
        ...
        this.createProgress = function (filename,filepath,filesize) {
            ...
            if(s.showFileCounter)
            	fileNameStr = obj.fileCounter +"-111-"+ s.fileCounterStyle + filename;
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for(var i = 0; i < files.length; i++) {
                ...
                var pd = new createProgressDiv(obj, s);
                var fileNameStr = "";
                if(s.showFileCounter) fileNameStr = obj.fileCounter +"-222-"+ s.fileCounterStyle + files[i].name
        ...
        //3274
        //easyphoto_string = '<a href="" id="thumb-image' + obj.fileCounter + '" data-toggle="image" class="img-thumbnail">';
        //easyphoto_string += '<img src="/image/catalog/easyphoto/' + files[i].name + '" data-placeholder="/image/cache/no_image-100x100.png">';
        //easyphoto_string += '</a>';
        //easyphoto_string += '<input type="hidden" name="product_image[' + obj.fileCounter + '][image]" value="' + files[i].name + '" id="input-image' + obj.fileCounter + '">';
        //3274

        easyphoto_string = "";

				pd.filename.html(easyphoto_string);
				//pd.filename.html(fileNameStr);
        ...
        function updateFileCounter(s, obj) {
            if(s.showFileCounter) {
                ...
                $(obj.container).find(".ajax-file-upload-filename").each(function (i, items) {
                    ...
                    var name = count + "-333-" + s.fileCounterStyle + arr[1];
                    $(this).html(name);
                    count--;
                });
            }
            ...
            fileInput.change(function () {
                ...
                if(s.serialize && feature.fileapi && feature.formdata) //use HTML5 support and split file submission
                {
                    ...
                    for(var i = 0; i < fileArray.length; i++) {
                        if(s.showFileCounter) fileList += obj.fileCounter + "-444-" + s.fileCounterStyle + fileArray[i] + "<br>";
