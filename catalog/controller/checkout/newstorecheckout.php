<?php 
class ControllerCheckoutNewstoreCheckout extends Controller { 
	...
	public function index() { 
		...
		$data['breadcrumbs'] = array();
      	...
      	$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('checkout/newstorecheckout', '', 'SSL'),
        	'separator' => $this->language->get('text_separator')
      	);
	...
	public function login_validate($data = array()) {
		...
		if ($this->customer->isLogged()) {
			$json['redirect'] = $this->url->link('checkout/newstorecheckout', '', true);			
		}
	...
	public function shipping_method($render = true, &$data = array()) 
	{
		...
		if ($render !== false)	
		{		
			$this->response->setOutput($this->load->view('checkout/newstoreshipping_method', $data));
		}
  	}
	...
	public function payment_address($render = true, &$data  = array()) {
  	...
  	public function payment_method($render = true, &$data = array()) {
		...
		if ($render !== false)
		{
			 $this->response->setOutput($this->load->view('checkout/newstorepayment_method', $data));
		}
  	}
	...
	public function cart($render = true, &$data = array()) 
	{
		...
		// Gift Voucher
			...
			$this->response->setOutput($this->load->view('checkout/newstorecart', $data));
	}
